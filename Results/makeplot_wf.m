...
a = double(data.a);
fstar = sum(-log(1./(a + data.sol)));
fval_avg = sum(-log(1./(repmat(a,1,Ko) + var.x_avg)),1);
fval_iter = sum(-log(1./(repmat(a,1,Ko) + double(var_fp.x))),1);
feas_avg = ones(1,data.N)*var.x_avg - double(data.pw);
feas_iter = ones(1,data.N)*double(var_fp.x) - double(data.pw);
...
indexOfInterest = [(Ko-100) : Ko]; % range of t near perturbation
hold on;
